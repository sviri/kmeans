...
Our online algorithm generates $\tilde{O}(k)$ clusters whose $k$-means cost is $\tilde{O}(\OPT)$ where $\OPT$ is the optimal $k$-means cost using $k$ clusters.
...
In contrast, the online model of computation does not allow to postpone the clustering decisions. 
That is, points arrive one by one in a potentially unbounded data stream.  
...
We require the algorithm outputs a single cluster identifier for each point online while using space and time at most logarithmic in the length of the stream.
...
Consider the stream of one dimensional scalars $x_1 = 0$, $x_2 = 1$ and $x_3 = c$ or $x_3 = 1/c$ for some $c \gg 1$.
...
In this work we consider two online $k$-means models. In the {\it semi-online} model we assume that we know the total number of vectors that we are going to process.  We design an algorithm that finds an approximate solution with at most $O(k\log n \log (nc_{max}/c_{min}))$ clusters in expectation and has objective value at most constant times the optimal value of the optimal  offline  solution for the $k$-means problem (in expectation). Where $c_{min}$ is the minimal possible distance between two vectors in our instance and $c_{max}$ is the maximal distance between a vector a its cluster center in the otpimal solution.
...
The algorithm uses ideas from online facility location algorithm of Meyerson \cite{Meyerson01}.
...
By doing that the, the algorithm is ``encouraged" to open many facilities (centers) which keeps the service costs low.
...
\STATE $f_1 \gets$ lower bound on eventual cost
\FOR {$i \in 1,\ldots, n$}
	...
	\ENDIF
\ENDFOR
\caption{Online $k$-means}\label{alg1}
\end{algorithmic}
\end{algorithm}
...
Let $A^*_i=W^*_i/|C^*_i|$ be the average distance to the cluster center from the vectors in the $i$-th optimal cluster. We define the partition of the cluster $C_i^*$ into subsets that we call {\it rings}: 
\begin{eqnarray}
...
dataset		&	nnz		& $n$	&	$d$	&	sparse? 	\\ \hline
20news-binary	&	2.44E+6	&	1.88E+4	&	6.12E+4	&	yes			\\ \hline %\cite{20news} 
adult			&	5.86E+5	&	4.88E+4	&	1.04E+2	&	yes			\\ \hline
ijcnn1		&	3.22E+5	&	2.50E+4	&	2.10E+1	&	no			\\ \hline %\cite{ijcnn1}
letter			&	2.94E+5	&	2.00E+4	&	1.50E+1	&	no			\\ \hline
magic04		&	1.71E+5	&	1.90E+4	&	9.00E+0	&	no		 	\\ \hline
maptaskcoref	&	6.41E+6	&	1.59E+5	&	5.94E+3	&	yes		 	\\ \hline
nomao		&	2.84E+6	&	3.45E+4	&	1.73E+2	&	no		 	\\ \hline
poker		&	8.52E+6	&	9.47E+5	&	9.00E+0	&	no		  	\\ \hline
shuttle		&	2.90E+5	&	4.35E+4	&	8.00E+0	&	no		 	\\ \hline
skin			&	4.84E+5	&	2.45E+5	&	2.00E+0	&	no		 	\\ \hline
vehv2binary	&	1.45E+7	&	2.99E+5	&	1.04E+2	&	no		 	\\ \hline
w8all			&	7.54E+5	&	5.92E+4	&	2.99E+2	&	yes		 	\\ \hline
\end{tabular}
\end{center}
...
Then, we invoked $k$-means++ using $k_{actual}$ many clusters and computed its cost, $f_{kmpp}$.
